...
if (!isset($_SESSION['user']))
	header("Location: ../../login.php?x=2"); #x=2 debe estar logueado

$objCon = Conexion();
$sql = "select * from empleado where Documento_emp='$_SESSION[user]'";
$res = $objCon->query($sql);
$existe = $res->fetch_object();

$sql2 = "select * from empleado where id_empleado='$_REQUEST[id]'";
$res2 = $objCon->query($sql2);

$eliminar = $res2->fetch_object();

if ($existe->Admin == "no") {
	header("Location: ../../usuario/Vista/index.php?x=1"); #x=1 debe ser admin
}
				...
				<div class="container-fluid">
					...
					<div class="offcanvas offcanvas-start bg-dark" style="color:white;" tabindex="-1" id="offcanvas" aria-labelledby="offcanvasLabel">
						<div class="offcanvas-header bg-light" style="color:black;">
							...
							<h3>Andres Felipe Obando Gonzalez</h3>
			...
			<article class="col-xs-12 col-sm-10 col-md-9 col-lg-9 col-xl-10 img-fluid" id="fondoCentral">
				<div class="container-fluid">
					...
					<div class="container-fluid">
						...
						<h4>Ver empleado a Eliminar:</h4><br />
						<div class="table-responsive col-xs-10 col-lg-11  col-xl-11">
							<table class="table table-striped table-bordered table-hover" style="border:2px solid;">
								...
								<tbody style="background:#FAF5F7;">
									<tr>
										<td><?php echo $eliminar->id_empleado ?></td>
										<td><?php echo $eliminar->Nombre_emp ?></td>
										<td><?php echo $eliminar->Apellido_emp ?></td>
										<td><?php echo $eliminar->Documento_emp ?></td>
										<td><?php echo $eliminar->Edad_emp ?></td>
										<td><?php echo $eliminar->Salario_emp ?></td>
										<td><?php echo $eliminar->Direccion_emp ?></td>
										<td><?php echo $eliminar->Correo_emp ?></td>
										<td><?php echo $eliminar->Telefono_emp ?></td>
										<td><?php echo $eliminar->Horario_emp ?></td>
										<td><?php echo $eliminar->Estado_emp ?></td>
										<td><?php echo $eliminar->Cargo_emp ?></td>
										<td><?php echo $eliminar->Area_emp ?></td>
										<td><?php echo $eliminar->Funciones_emp ?></td>
										<td><?php echo $eliminar->Profesion_emp ?></td>
										<td><?php echo $eliminar->Diasdes_emp ?></td>
										<td><?php echo $eliminar->Contraseña_usuario ?></td>
										<td><?php echo $eliminar->NivelEducativo_emp ?></td>
										<td><?php echo $eliminar->FechaAfiliacion_emp ?></td>
										<td><?php echo $eliminar->Admin ?></td>
									</tr>
								</tbody>
							</table>
						</div>
						<br />
						<p class="col-xs-12 col-sm-10 col-md-8 col-lg-7" style="border:1px solid;background:#FAF5F7;padding:10px;">Si procede con esta accion el empleado
							<strong><?php echo $eliminar->Nombre_emp . " " . $eliminar->Apellido_emp ?></strong> sera eliminado de la base de datos
							de la compañia, eliminando asi sus archivos, afiliaciones, y demas datos que esten vinculados con la compañia,
							esta seguro de desear proceder?, esta accion es permanente (eso es mucho tiempo).
						</p>
						<br />
						<form action="../Controlador/eliminar.php" method="POST" class="col-sm-9">
							<fieldset>
								<input type="hidden" name="x" value="0">
								<input type="hidden" name="id" value="<?php echo $eliminar->id_empleado ?>">
								<input type="hidden" name="elim" value="<?php echo $_SESSION['user'] ?>">
								<input type="hidden" name="doc" value="<?php echo $eliminar->Documento_emp ?>">
